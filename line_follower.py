...
class LineFollower:
    ...
    def move_while_on_track(self, direction, speed):
        ...
        self.steering_drive.on_for_seconds(0, SpeedPercent(-speed), LineFollower.TIME_STEP * LineFollower.STEPS_PER_SECOND * 10, False, False)
        while self.on_track():
            print(self.last_brightness)
            effective_direction = direction
            if self.last_brightness < brightness - LineFollower.VEERING_THRESHOLD:
                effective_direction = self.last_search_direction * 10
                print('Veering ' + direction_name(self.last_search_direction))
            #self.steering_drive.on_for_seconds(effective_direction, SpeedPercent(-speed), LineFollower.TIME_STEP, False)
            brightness = self.last_brightness
    ...
    def move_until_on_track(self, direction, speed, max_steps):
        i = 0
        self.steering_drive.on_for_seconds(direction, SpeedPercent(-speed), LineFollower.TIME_STEP * max_steps, False, False)
        while (self.steering_drive.is_running or self.steering_drive.is_ramping) and not self.on_track() and self.color_sensor.color != 6:
            #self.steering_drive.on_for_seconds(direction, SpeedPercent(-speed), LineFollower.TIME_STEP, False)
            i = i + 1
            #if i >= max_steps:
            #    break
    ...
    def turn_and_seek(self, max_turn_steps, turn_direction):
        ...
        print('Turn ' + name)
        result = self.move_until_on_track(100 * turn_direction, LineFollower.SEARCH_SPEED, max_turn_steps)
        if result == -1:
            return True
        print('Turn back')
        self.steering_drive.on_for_seconds(100 * turn_direction * -1, SpeedPercent(-LineFollower.SEARCH_SPEED), LineFollower.TIME_STEP * result, False)
        return False
    ...
    def start(self):
        direction = 1
        max_turn_steps = LineFollower.SEARCH_MAX_STEPS
        yellow_count = 0

        #We need to go forward first
        #self.steering_drive.on_for_seconds(0, SpeedPercent(-2 * LineFollower.FORWARD_SPEED), LineFollower.TIME_STEP * 8, False)
...
def main():
    ...
    follower = LineFollower(movesteering, colorsensor)
    follower.start()
